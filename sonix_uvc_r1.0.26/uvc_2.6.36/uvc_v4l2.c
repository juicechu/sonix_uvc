...
#endif
{
...
#if SONIX_FW_CHECK	
		if(!stream->dev->is_fw_correct)
		{
			//printk(KERN_INFO "skip ioctl command\n");
			return -EPERM; 
		}
#endif
	...
	case UVCIOC_STILL_REQBUF:
	{
		...
		ret = uvc_alloc_still_buffer(&stream->still_queue, bufsize
								, stream->queue.total_size);
...
static int uvc_v4l2_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if( still_queue->mmap_offset && (still_queue->mmap_offset>> PAGE_SHIFT) == vma->vm_pgoff )
	{
		uvc_trace(UVC_TRACE_CALLS, "%s:still\n", __FUNCTION__);
		mutex_lock(&still_queue->mutex);
		buffer = &still_queue->buffer;

		if (size != still_queue->buf_size) {
			ret = -EINVAL;
			goto done_still;
		}

		vma->vm_flags |= VM_IO;		
		addr = (unsigned long)still_queue->mem;
		while (size > 0) {
...
done_still:		
	...
	uvc_trace(UVC_TRACE_CALLS, "%s:video\n", __FUNCTION__);
