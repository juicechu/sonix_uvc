...
int uvc_xu_ctrll_ReadChip(struct uvc_device *dev, char *chip)
{
	...
	*chip = CHIP_NONE;
	...
	{
		if(term->id == xctrl.unit)
		{
			if(!uvc_entity_match_guid(term,sonix_xu_sys_ctrls[0].entity))
			{
...
int uvc_xu_ctrl_CheckFw(struct uvc_device *dev)
{
	...
	list_for_each_entry(term, &dev->entities, list)
	{
		if(term->id == xctrl.unit)
		{
			if(!uvc_entity_match_guid(term,sonix_xu_usr_ctrls[0].entity))
			{
				DbgPrint(My_TRACE_XU,"guid doesn't match (%pUl)\n", term->extension.guidExtensionCode);
				return -EINVAL;
			}		
	...
	if(strncmp(xctrl.data, "SpecialVer" , 10)==0)
		return 1;
	else 
		return 0;
