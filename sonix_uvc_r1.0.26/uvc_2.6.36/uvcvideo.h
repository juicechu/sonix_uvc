...
#if LINUX_VERSION_CODE == KERNEL_VERSION (2,6,30) ||  LINUX_VERSION_CODE == KERNEL_VERSION (2,6,28)
...
struct uvc_video_queue {
	...
	unsigned int total_size;					//for still image offset
...
struct uvc_device {
	...
	unsigned char SONiX_Chip, valid_device, is_fw_correct;
};
...
extern void uvc_queue_init(struct uvc_video_queue *queue,
		enum v4l2_buf_type type);
extern void uvc_queue_still_init(struct uvc_still_image_queue *queue, enum v4l2_buf_type type);
extern int uvc_alloc_buffers(struct uvc_video_queue *queue,
		unsigned int nbuffers, unsigned int buflength);
extern int uvc_alloc_still_buffer(struct uvc_still_image_queue *still_queue, unsigned int buflength, 
				unsigned int mmap_offset);
extern int uvc_free_buffers(struct uvc_video_queue *queue);
extern int uvc_free_still_buffers(struct uvc_still_image_queue *still_queue);
...
extern int uvc_queue_still_buffer(struct uvc_still_image_queue *still_queue,
	struct v4l2_buffer *v4l2_buf);
extern int uvc_queue_buffer_resume(struct uvc_video_queue *queue, int* buf_index);
...
extern void uvc_still_queue_cancel(struct uvc_still_image_queue *still_queue, int disconnect);
extern struct uvc_buffer *uvc_queue_next_buffer(struct uvc_video_queue *queue,
		struct uvc_buffer *buf);
extern void uvc_queue_finish_still_buffer(struct uvc_still_image_queue *still_queue,
		struct uvc_buffer *buf);
...
extern int uvc_xu_ctrl_CheckFw(struct uvc_device *dev);
