...
int uvc_xu_ctrl(struct uvc_device *dev, unsigned char query, unsigned char unit, unsigned char selector, unsigned char *data, int len)
{
	struct uvc_entity *entity;
	struct uvc_control *ctrl;
	int ret, i, found=0;
	...
	if (!found) {
		uvc_trace(UVC_TRACE_CONTROL, "Control %pUl/%u not found.\n",
			entity->extension.guidExtensionCode, selector);
		return -ENOENT;
	}

	ret = uvc_query_ctrl(dev, query, unit, dev->intfnum, selector, data, len);
...
int uvc_xu_ctrl_CheckFw(struct uvc_device *dev)
{
	int ret = 0;
	__u8 data[11];
	DbgPrint(My_TRACE_XU,"uvc_xu_ctrl_CheckFw ==>\n");
	ret = uvc_xu_ctrl(dev, UVC_GET_CUR, 4, 10, data, sizeof(data));
	if(ret < 0)
	{
		DbgPrint(My_TRACE_XU,"uvc_xu_ctrl_query(get fw string) fail - (%i) == \n",ret);
		return ret;
	}
	if(strncmp(data, "SpecialVer" , 10)==0)
		return 1;
	else 
		return 0;
...
int uvc_xu_ctrll_ReadChip(struct uvc_device *dev, char *chip)
{
	...
	*chip = CHIP_NONE;
	...
	// Dummy Write
	ret = uvc_xu_ctrl(dev, UVC_SET_CUR, 3, XU_SONIX_SYS_ASIC_READ, ctrldata, 4);
	if(ret < 0)
	{
		DbgPrint(My_TRACE_XU,"uvc_xu_ctrl_query(Dummy Write) - (%i) == \n",ret);
		return ret;
	...
	// Asic Read
	ctrldata[3] = 0x00;
	ret =  uvc_xu_ctrl(dev, UVC_GET_CUR, 3, XU_SONIX_SYS_ASIC_READ, ctrldata, 4);
